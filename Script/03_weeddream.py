...
# Randomly select weights for the selected layers.
from random import sample 
# Change here to use other combinations of layers.
layers = [2, 6, 9]
n_layers = len(layers)
layers = ['mixed' + str(i) for i in layers]
# weights = sample(list(np.arange(1.0, 10.5, 0.5)), n_layers)
weights = [2, 5, 4]
layer_settings = { layers[i]: weights[i] for i in range(n_layers) }
...
keras.preprocessing.image.save_img('NiceDreams/' + result_prefix + '.png', deprocess_image(img.numpy()))
